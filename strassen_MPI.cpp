...
void runStrassen(const vector<double>& A, const vector<double>& B, vector<double>& C, int size) {
    if (size <= 0) return;
    strassenRecursive(A, B, C, size);
}
...
void workerLoop() {
    ...
    runStrassen(left, right, result, header.blockSize);
...
void distributedStrassen(const vector<double>& A_pad, const vector<double>& B_pad,
    ...
    if (worldSize == 1 || size <= DISTRIBUTED_THRESHOLD) {
        runStrassen(A_pad, B_pad, C_pad, size);
        return;
    }
    ...
    for (int task = 0; task < 7; ++task) {
        ...
        if (owner == 0) {
            runStrassen(leftOps[task], rightOps[task], results[task], newSize);
        } else {
