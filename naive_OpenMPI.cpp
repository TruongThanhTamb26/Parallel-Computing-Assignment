...
void multiplyNaiveLocal(const std::vector<double>& localA, int localRows, int commonDim,
    ...
    // localRows: number of rows this rank owns
    // commonDim: columns of A (= rows of B)
    // colsB: columns of B
...
int main(int argc, char** argv) {
    ...
    if (rank == 0) {
        #ifdef _OPENMP
            std::cout << "Hybrid Naive (MPI + OpenMP) -- Processes: " << size
                      << " | Threads per process: " << omp_get_max_threads() << std::endl;
        #else
            std::cout << "Hybrid Naive (MPI build without OpenMP) -- Processes: " << size << std::endl;
        #endif
    }
    ...
    if (rank == 0) {
        ...
        appendReport("Naive", "Hybrid MPI+OpenMP", r1, c1, r2, c2, elapsed.count(), checksum, size, threadCount);
